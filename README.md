...
离线版《星空》技能规划网页。可直接访问 [网页版本](https://mqzh.science/sf_skills/) 或者本地克隆仓库后直接双击 `index.html` 即可在本地浏览器中打开，不需要 Python 服务。

Offline Starfield skill planner. Directly visit [Webpage](https://mqzh.science/sf_skills/) or clone the repo locally and double-click `index.html` to open it locally in your browser with no web server required.
